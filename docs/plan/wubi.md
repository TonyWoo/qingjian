...
**五笔候选排在同一次查询的拼音候选前面。** 理由：编码是精确的（四码定字），
而混输的典型用法是「主要用五笔，打不出的字才打拼音」。
...
对**主要用拼音、顺手开着五笔**的人，`kai` 的首选会被 中共党员 顶掉。2026-09-16 与维护者确认
**保持形码在前**，不改成拼音优先。备选方案（拼音优先、五笔兜底）只需换 `query_mixed` 里两段的顺序。
